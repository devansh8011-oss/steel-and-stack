...
export const IsometricBrowser: React.FC = () => {
  ...
  return (
    <div className="w-full relative flex flex-col items-center select-none py-2">
        ...
        >
          ...
          <div className="p-4 sm:p-5 flex-1 bg-slate-50 flex flex-col justify-between overflow-hidden">
            ...
            <div className="grid grid-cols-3 gap-1.5 sm:gap-3 py-1.5 sm:py-2 font-mono">
              <div className="bg-white p-1.5 sm:p-2.5 rounded-lg sm:rounded-xl border border-slate-200/90 text-center shadow-xs">
                <span className="text-slate-500 block text-[8px] sm:text-[9px] font-medium uppercase">LCP Speed</span>
                <strong className="text-emerald-600 font-bold text-xs sm:text-sm">0.62s</strong>
                <span className="text-[7px] sm:text-[8px] text-slate-400 block mt-0.5">&lt; 0.8s Target</span>
              </div>
              <div className="bg-white p-1.5 sm:p-2.5 rounded-lg sm:rounded-xl border border-slate-200/90 text-center shadow-xs">
                <span className="text-slate-500 block text-[8px] sm:text-[9px] font-medium uppercase">Input (INP)</span>
                <strong className="text-emerald-600 font-bold text-xs sm:text-sm">12ms</strong>
                <span className="text-[7px] sm:text-[8px] text-slate-400 block mt-0.5">Zero Lag</span>
              </div>
              <div className="bg-white p-1.5 sm:p-2.5 rounded-lg sm:rounded-xl border border-slate-200/90 text-center shadow-xs">
                <span className="text-slate-500 block text-[8px] sm:text-[9px] font-medium uppercase">Lighthouse</span>
                <strong className="text-brand-orange-600 font-bold text-xs sm:text-sm">100/100</strong>
                <span className="text-[7px] sm:text-[8px] text-slate-400 block mt-0.5">Top Tier</span>
              </div>
            </div>

            {/* Executive Pipeline Simulation Block */}
            <div className="bg-slate-900 text-slate-200 p-2.5 sm:p-3.5 rounded-xl sm:rounded-2xl font-mono text-[9px] sm:text-[10px] leading-relaxed border border-slate-800 shadow-inner space-y-1 sm:space-y-1.5">
              ...
              <div className="text-slate-300 flex justify-between pt-0.5">
                ...
                <span className="text-white font-bold truncate ml-2">devansh8011@gmail.com</span>
              </div>
            ...
            {/* Footer Status Bar */}
            <div className="pt-2 flex items-center justify-between border-t border-slate-200 text-[8px] sm:text-[9px] font-mono text-slate-600">
              <span className="truncate">Mobile-First • Zero Shift</span>
              <span className="bg-brand-orange-500 text-white px-2 py-0.5 rounded-full font-bold text-[7px] sm:text-[8px] uppercase tracking-wider flex-shrink-0">
                Turnkey Ready
              </span>
            </div>
          </div>
      ...
      <div className="w-full max-w-xl bg-white border border-slate-200/90 rounded-2xl p-3 sm:p-5 shadow-subtle-card mt-3 flex flex-col sm:flex-row items-center justify-between gap-3">
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-brand-orange-50 border border-brand-orange-200 flex items-center justify-center text-brand-orange-600 flex-shrink-0">
            <Gauge className="w-4 h-4 sm:w-5 sm:h-5" />
          </div>
          <div>
            <span className="text-[9px] sm:text-[10px] font-mono font-bold uppercase tracking-widest text-brand-orange-600 block">
              Core Web Engine
            </span>
        ...
        <div className="flex items-center justify-between gap-1 bg-slate-100 p-1 rounded-xl border border-slate-200 w-full sm:w-auto">
          <button
            onClick={() => setDeviceView('desktop')}
            type="button"
            className={`flex-1 sm:flex-initial justify-center px-2.5 py-1.5 rounded-lg text-[11px] sm:text-xs font-mono transition-all flex items-center gap-1 sm:gap-1.5 ${
              deviceView === 'desktop'
